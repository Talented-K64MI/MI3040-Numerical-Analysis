...
def color_frame(master, color, name):
    from tkinter import ttk, StringVar
    frame = ttk.Frame(master)
    color_var = StringVar()
    entry = ttk.Entry(frame, textvariable=color_var, width=18, state='readonly')
    label = ttk.Label(frame, text=name, background=color)
    entry.grid(row=0, column=0)
    label.grid(row=0, column=1)
    return frame, color_var, color
...
def show_colors():
    ...
    root = Tk()
    root.title('常用颜色')
    widgets = [color_frame(root, color, name)
               for color, name in color_dict.items()]
    row = 0
    column = 0
    for k, (frame, color_var, color) in enumerate(widgets):
        if k % 6 == 0:
            row += 1
            column = 0
